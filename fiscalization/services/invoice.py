...
def make_invoice(data, company_p12_certificate, certificate_password):
    ...
    iic_input = ''
    invoice_iic = ''
    invoice_iic_signature = 'iic.generate_iic(iic_input=iic_input)'
    ...
    invoice_issuer_nuis = data.get('issuer_nuis')
    ...
    if not online:
        header_dictionary['SubseqDelivType'] = 'NOINTERNET'
    ...
    if (invoice_is_simplified_inv):
        invoice_dictionary['IsSimplifiedInv'] = 'true'
    else:
        invoice_dictionary['IsSimplifiedInv'] = 'false'

    invoice_issue_date_time = data.get('invoice_issue_date_time')
    # if (not invoice_issue_date_time):
    #     invoice_issue_date_time = datetime.datetime.utcnow().replace(
    #         tzinfo=from_zone).astimezone(to_zone).replace(
    #         microsecond=0).isoformat()
    #
    # if not data.get('iic_code'):
    #     if (invoice_issue_date_time):
    #         invoice_dictionary['IssueDateTime'] = invoice_issue_date_time
    # else:
    invoice_dictionary['IssueDateTime'] = data.get('invoice_issue_date_time')
    ...
    if (invoice_tot_price_wo_vat):
        print("invoice_tot_price_wo_vat 000000000000000000000", invoice_tot_price_wo_vat)
    ...
    invoice_dictionary['TotPrice'] = str("{:.2f}".format(float(invoice_tot_price)))
    invoice_tot_price = str("{:.2f}".format(float(invoice_tot_price)))
    # else:
    #     invoice_tot_price = str(data.get('amount_total'))
    ...
    # print("data", data)
    if not data.get('iic_code'):
        invoice_iic = iic.generate_iic(iic_input=iic_input, company_p12_certificate=company_p12_certificate,
                                       certificate_password=certificate_password)
    else:
        invoice_iic = data.get('iic_code')

    invoice_iic_signature = iic.generate_iic_signature(iic_input=iic_input,
                                                       company_p12_certificate=company_p12_certificate,
                                                       certificate_password=certificate_password)

    invoice_dictionary['IIC'] = invoice_iic

    invoice_dictionary['IICSignature'] = invoice_iic_signature

    invoice_is_reverse_charge = data.get('is_reverse_charge')
    if (invoice_is_reverse_charge):
        invoice_dictionary['IsReverseCharge'] = invoice_is_reverse_charge

    invoice_pay_deadline = data.get('PayDeadline')
    if (invoice_pay_deadline):
        invoice_dictionary['PayDeadline'] = invoice_pay_deadline
    ...
    # correctiveinv_iic_ref = 'C34C51045A777042CBBD1AD85BB6946F'
    if data.get("is_corrective_invoice") == 'true':
        correctiveinv_iic_ref = data.get('invoice_corrective_iic_ref')
        if correctiveinv_iic_ref:
            correctiveinv_dictionary['IICRef'] = correctiveinv_iic_ref

        # correctiveinv_issue_date_time = '2021-05-18T05:49:14+02:00'
        correctiveinv_issue_date_time = data.get('invoice_corrective_issue_date_time')
        if (correctiveinv_issue_date_time):
            correctiveinv_dictionary[
                'IssueDateTime'] = correctiveinv_issue_date_time

        # correctiveinv_type = 'CORRECTIVE'
        correctiveinv_type = data.get('invoice_corrective_type')
        if (correctiveinv_type):
            correctiveinv_dictionary['Type'] = "CREDIT"

    # Invoice->BadDebtInv
    baddebtinv_iic_ref = data.get('BadDebtInv-IICRef')
    if (baddebtinv_iic_ref):
    ...
    # suminviicref_iics = data.get('sum_inv_iic_refs')
    # if suminviicref_iics:
    ...
    supplydateorperiod_start = data.get('SupplyDateOrPeriod-Start')
    if (supplydateorperiod_start):
        supplydateorperiod_dictionary['Start'] = supplydateorperiod_start

    supplydateorperiod_end = data.get('SupplyDateOrPeriod-End')
    if (supplydateorperiod_end):
        supplydateorperiod_dictionary['End'] = supplydateorperiod_end
    ...
    if not buyer_dictionary.get("IDNum"):
        buyer_dictionary = {}
    ...
    for i in data['lines']:
        ...
        i_n = getattr(i, 'invoice_item_name')
        if i_n:
            i_dictionary['N'] =  i_n[:50]

        product_id = getattr(i, 'product_id')

        i_c = product_id.barcode or product_id.default_code
        if i_c:
            i_dictionary['C'] = i_c
        ...
        i_q = str("{:.2f}".format(getattr(i, 'qty')))
        if i_q:
            i_dictionary['Q'] = i_q

        tax_ids = getattr(i, 'tax_ids')
        price_unit = getattr(i, 'price_unit')
        currency = getattr(i, 'product_id').currency_id
        ...
        i_upb = str("{:.2f}".format(total_excluded))
        if (i_upb):
            i_dictionary['UPB'] = i_upb

        unit_vat_rate = getattr(getattr(i, 'tax_ids', {}), 'amount', 0) / 100

        i_upa = str("{:.2f}".format(total_included))
        if (i_upa):
        ...
        # i_r = getattr(i, 'I-R', None)
        # if (i_r):
        #     i_dictionary['R'] = str(i_r)
        #
        # i_rr = getattr(i, 'I-RR', None)
        # if (i_rr):
        #     i_dictionary['RR'] = str(i_rr)

        i_pb = str("{:.2f}".format(getattr(i, 'price_subtotal')))
        if (i_pb):
            i_dictionary['PB'] = i_pb

        # i_ex = getattr(i, 'I-EX', None)
        # if (i_ex):
        #     i_dictionary['EX'] = str(i_ex)
        is_exempt = False
        if i.tax_ids_after_fiscal_position.amount == 0:
            # if data["is_export"]:
            #     i_dictionary['EX'] = "EXPORT_OF_GOODS"
            if i.tax_ids_after_fiscal_position.exempt_code:
                i_dictionary['EX'] = i.tax_ids_after_fiscal_position.exempt_code
                is_exempt = True

        i_va = str("{:.2f}".format(getattr(i, 'price_subtotal_incl') - getattr(i, 'price_subtotal')))
        if i_va and not is_exempt:
            i_dictionary['VA'] = i_va

        i_vr = str("{:.2f}".format(unit_vat_rate * 100))
        ...
        i_pa = str("{:.2f}".format(getattr(i, 'price_subtotal_incl')))
        if (i_pa):
    ...
    # same_taxes = set([line['tax_ids']['amount'] for line in data['lines']])
    taxes = []
    for line in data['lines']:
        if line.tax_ids:
            if line.tax_ids.amount == 0 and line.tax_ids.exempt_code == 'TYPE_1':
                taxes.append("exempt_1")
            elif line.tax_ids.amount == 0 and line.tax_ids.exempt_code == 'TYPE_2':
                taxes.append("exempt_2")
            else:
                taxes.append(line.tax_ids.amount)

    # taxes = [line.tax_ids.amount for line in data['lines'] if line.tax_ids]
    print("taxes>>>>>>>>>", taxes)
    same_taxes = {}
    # _logger.info(">>>>>>>>>>>>>>>>>???? TAX %s" % (taxes))
    for tax in taxes:
        tax = str(tax)
        if tax not in same_taxes:
            same_taxes.update({tax: [tax]})
        else:
            same_taxes[tax].append(tax)

    list_sametaxes_dictionaries = []
    print("Same_taxes", same_taxes)
    # _logger.info(">>>>>>>>>>>>>>>>>Same TAX %s" % (same_taxes))
    if invoice_is_issuer_in_vat:
        empty_taxes = ['0.0' for line in data['lines'] if not line.tax_ids]
        print("empty_taxes", empty_taxes)
        if empty_taxes:
            if '0.0' in same_taxes:
                same_taxes['0.0'].extend(empty_taxes)
            else:
                same_taxes.update({'0.0': empty_taxes})
        print("Same_taxes again", same_taxes)
        for tax_key, vals in same_taxes.items():
            sametax_dictionary = {}
            exempt = False
            if tax_key == "exempt_1":
                tax_key = '0.0'
            ...
            if tax_key == "exempt_2":
                ...
                exempt = "exempt_2"
            tax = float(tax_key)
            sametax_num_of_items = str(len(vals))
            # sametax_num_of_items = str(int(sum([getattr(line, 'qty', 0) for line in data['lines'] if line['tax_ids']['amount'] == tax])))
            if (sametax_num_of_items):
                sametax_dictionary['NumOfItems'] = sametax_num_of_items
            price_sum = 0

            for line in data['lines']:
                if line['tax_ids'] and line['tax_ids']['amount'] == tax and tax_key not in ["0.0", "exempt_1",
                                                                                            "exempt_2"]:
                    price_sum += getattr(line, 'price_subtotal')
                if tax_key == "0.0" and not exempt:
                    if line['tax_ids'] and line['tax_ids']['amount'] == tax and not line['tax_ids'][
                        'exempt_code'] or not line['tax_ids']:
                        price_sum += getattr(line, 'price_subtotal')
                if exempt == "exempt_1":
                    if line['tax_ids'] and line['tax_ids']['amount'] == tax and line['tax_ids'][
                        'exempt_code'] == 'TYPE_1':
                        price_sum += getattr(line, 'price_subtotal')
                if exempt == "exempt_2":
                    if line['tax_ids'] and line['tax_ids']['amount'] == tax and line['tax_ids'][
            ...
            sametax_price_bef_vat_float = price_sum
            # sametax_price_bef_vat_float = sum([getattr(line, 'price_subtotal') for line in data['lines'] if
            #      line['tax_ids'] and line['tax_ids']['amount'] == tax])
            sametax_price_bef_vat = str("{:.2f}".format(sametax_price_bef_vat_float))
            if sametax_price_bef_vat:
                sametax_dictionary['PriceBefVAT'] = sametax_price_bef_vat

            sametax_vat_rate = str("{:.2f}".format(tax))
            if sametax_vat_rate and not exempt:
                sametax_dictionary['VATRate'] = sametax_vat_rate

            # sametax_exempt_from_vat = data.get('invoice_same_tax_type_of_exempt_from_vat')
            if exempt:
                sametax_dictionary['ExemptFromVAT'] = "TYPE_1" if exempt == "exempt_1" else "TYPE_2"

            sametax_vat_amt = str("{:.2f}".format(sum([getattr(line,
                                                               'price_subtotal_incl') - getattr(
                line, 'price_subtotal') for line in data['lines'] if
                                                       line['tax_ids'][
                                                           'amount'] == tax])))
            if sametax_vat_amt and not exempt:
                sametax_dictionary['VATAmt'] = sametax_vat_amt
            list_sametaxes_dictionaries.append(sametax_dictionary)
    # _logger.info(">>>>>>>>>>>>>>>>>Same TAX list_sametaxes_dictionaries %s" % (list_sametaxes_dictionaries))
    ...
    # print("final Invoice  dictornory",invoice_dictionary)
    ...
    if suminviicref_iics:
        suminviicrefs_subelement = SubElement(invoice_subelement,
                                              'SumInvIICRefs')
        for iic_refs in suminviicref_iics:
    ...
    # if invoice_is_issuer_in_vat exist then and only add sametax
    if list_sametaxes_dictionaries:
        sametaxes_subelement = SubElement(invoice_subelement, 'SameTaxes')
        for sametax_dictionary in list_sametaxes_dictionaries:
    ...
    return final_xml, iic_input, invoice_iic, invoice_iic_signature
