...
def build_iic_input(issuer_nipt, datetime_created, invoice_number,
                    business_unit_code, tcr_code, soft_code,
                    total_price):
    return issuer_nipt + '|' + datetime_created + '|' + invoice_number + '|' \
           + business_unit_code + '|' + tcr_code + '|' + soft_code + '|' + \
           total_price
...
def generate_iic_signature(iic_input, company_p12_certificate, certificate_password):
    ...
    # p12_location = os.getenv('P12_LOCATION')
    # password = os.getenv('PRIVATE_PASSWORD').encode('utf-8')
    # p12 = crypto.load_pkcs12(open(p12_location, 'rb').read(), password)
