...
class PartnerLedgerForeignCurrency(models.AbstractModel):
    ...
    def get_init(self,data,partner_id):
        ...
        if data['form']['date_from']:
            company_domain = ' and am.company_id = %d ' % self.env.company.id 
            partner_domain= ' and aml.partner_id = %d ' % partner_id.id
            date_domain =  """ and aml.date < '%s' """ % data['form']['date_from']
            curr_obj = self.env['res.currency']
            currency_domain =''
            if data['form']['currency_id'] and data['form']['currency_id'][0] == self.env.company.currency_id.id:
                currency_domain = ' and (aml.currency_id is null or aml.currency_id = %d)' % self.env.company.currency_id.id
            ...
            sql = """select aml.currency_id,
                            sum(case when aml.debit  != 0.0 and aml.currency_id is  not null then aml.amount_currency  
                            ...
                            abs(sum(case when aml.credit  != 0.0 and aml.currency_id is not null then aml.amount_currency
                                 when aml.credit  != 0.0 and aml.currency_id is null then aml.credit else 0.0 end )) credit,
                            sum(case when aml.debit  != 0.0 and aml.currency_id is  not null then aml.debit  
                                 when aml.debit  != 0.0 and aml.currency_id is null then aml.debit else 0.0 end) as debit_bc,
                            abs(sum(case when aml.credit  != 0.0 and aml.currency_id is not null then aml.credit
                                 when aml.credit  != 0.0 and aml.currency_id is null then aml.credit else 0.0 end )) credit_bc 
            ...
            for i in lines_init:
                dic[curr_obj.browse(i['currency_id'] or self.env.company.currency_id.id)] = {'debit':i['debit'],
                                                                                             'credit':i['credit'],
                                                                                             'debit_bc':i['debit_bc'],
                                                                                             'credit_bc':i['credit_bc']}
    ...
    def get_lines(self,data,partner_id):
        ...
        sql = """  
                ...
                aml.id ,
                aml.name,
                case when aml.debit  != 0.0 and aml.amount_currency != 0.0 then aml.amount_currency
                ...
                aml.debit debit_bc,
                aml.credit credit_bc,
                aml.currency_id,
                aml.amount_currency from 
                 res_partner rp 
                inner join account_move_line aml on rp.id = aml.partner_id %s %s %s %s
                inner join account_move am on am.id = aml.move_id %s %s
                inner join account_journal aj on aj.id = am.journal_id
                order by aml.currency_id,aml.date
 ...
 """ % (partner_domain,currency_domain,journal_domain,date_domain,company_domain,target_move)
        ...
        curr_obj = self.env['res.currency']
        for line in lines:
            line['currency_id'] = curr_obj.browse(line['currency_id']) if line['currency_id'] else self.env.company.currency_id
        ...
        for k,v in dic.items():
            final_list.append({'bal':bal[k] if k in bal else  {'debit':0.0,'credit':0.0, 'debit_bc': 0.0, 'credit_bc': 0.0},'cu':k,'lines':v})
        return final_list
    
    def get_converted(self, bal, cureency, date):
        t_cur = self.env.company.currency_id
        if date:
            converted_bal = cureency._convert(bal, t_cur, self.env.company, date)
        else:
            date = fields.Date.today()
            converted_bal = cureency._convert(bal, t_cur, self.env.company, date)
        
        return [converted_bal, t_cur]
    ...
    def _get_report_values(self, docids, data=None):
        ...
        return {
            ...
            'get_converted': self.get_converted,
