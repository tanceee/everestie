...
class pos_config(models.Model):
    ...
    allow_internal_locations = fields.Many2many("stock.location", "stock_config_loaction", "stock_location",
                                                "config_location", "Internal Locations",
                                                domain="[('usage', '=', 'internal'),('company_id','=',company_id)]")
    allow_picking_type_ids = fields.Many2many("stock.picking.type", "stock_picking_type_config_id", "config_id",
                                              "stock_type_id", "Picking Type",
    ...
    def create_stock_picking(self, client, picking_type, src, dest, state, product, company_id):
        ...
        for i in product:
            product_obj = self.env['product.product'].search(
                [('id', '=', i.get('product_id')), ('type', 'in', ['consu', 'product'])])
            if product_obj:
        ...
        if state == "done":
            pick.action_confirm()
            for stock_move_id in pick.move_ids_without_package:
